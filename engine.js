class GameEngine {
    ...
    init() {
        ...
        // Post-processing بعداً اضافه می‌شود
    ...
    setupPostProcessing() {
        try {
            // استفاده از DefaultRenderingPipeline برای پست-پردسینگ ساده
    ...
    createTerrain() {
        // ایجاد زمین ساده (بدون height map برای جلوگیری از خطا)
    ...
    createWater() {
        try {
            ...
            // بررسی آیا WaterMaterial موجود است
            if (typeof BABYLON.WaterMaterial !== 'undefined') {
                ...
                // Fallback material اگر WaterMaterial موجود نباشد
            ...
            console.warn("Water creation failed:", error);
        }
    }

    createVegetation() {
        // Create trees and vegetation
        for (let i = 0; i < 20; i++) { // تعداد کمتر برای عملکرد بهتر
            this.createTree(
                Math.random() * 180 - 90,
                Math.random() * 180 - 90
            );
